...
export default function CoreValues() {
  ...
  return (
    <section id="values" className="relative bg-gradient-to-b from-white to-sky-50/40 py-16">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {coreValues.map(({ title, description, Icon }) => (
            <div
              key={title}
              className="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"
            >
              <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 p-3 items-center justify-center rounded-lg bg-sky-50 text-sky-700 ring-1 ring-sky-200">
              ...
              <div className="pointer-events-none absolute -right-6 -bottom-6 h-24 w-24 rounded-full bg-sky-100 opacity-0 blur-2xl transition-opacity duration-300 group-hover:opacity-80" />
            </div>
          ))}
        </div>
      </div>
